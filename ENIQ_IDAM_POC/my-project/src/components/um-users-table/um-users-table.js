...
import { Button, Dialog, Tooltip, Menu, Checkbox,TextField,Tree,Link } from '@eui/base';
...
export default class UmUsersTable extends LitComponent {
   ...
   constructor() {
     ...
     this.dialog_eventlistener;
      ...
      this.data=[
        ...
        { col1: 'ClOGNEc', col2: 'John', col3: 'smith', col4: 'j.smith@yahoo.com', col5: 'NetAn', col6: 'NetAn Admin', col7: 'Enabled' },
       ];
  }
 ...
 didConnect(){
  ...
  //multi_panel_subtitle
  this.multi_panel_subtitle=`${this.data.length} Items | Filters applied - Clear | Last refreshed right now"`;
   //user table_columns
   this.table_columns=[
    { title: 'Username', attribute: 'col1', sortable: true, mandatory: true,
      cell: (row, column) => html`
      <div class='table__cell' part='username_cell'>
        <div part='username'>${row[column.attribute]}</div>
        <eui-icon part='menu' name="more"></eui-icon>        
      </div>`,resizable:true
    },
    { title: 'First name', attribute: 'col2', sortable: true,resizable:true},
    { title: 'Last name', attribute: 'col3', sortable: true, resizable:true},
    { title: 'E-mail', attribute: 'col4', sortable: true,resizable:true },
    { title: 'Application', attribute: 'col5', sortable: true,resizable:true },
    { title: 'Group', attribute: 'col6', sortable: true,resizable:true,width:"20%" },
    { title: 'Status', attribute: 'col7', sortable: true,resizable:false,
      cell: (row, column) => html`
          <div class='table__cell' part="status_cell">
          <eui-icon name=${row[column.attribute] == "Disabled"? "cross":"check"}
           color=${row[column.attribute] == "Disabled"? "red":"green"}></eui-icon>
  ...
  //filterApplications
  this.filterapps_data=[{"label":"AdminUI"},{"label": "AlarmCfg"}, {"label":"BI"}, {"label":"Launcher page"}, 
  {"label":"NetAn"}, {"label":"RHEL"},{"label":"Self Service"}, {"label": "TechPackIDE"},
  {"label":"User Management"},{"label":"Windows"}];
  
  //filterGroup
  this.filterGroup_data=[
    { "label": "AdminUI", "children": [{ "label": "AdminUI ReadOnly" },{ "label": "AdminUI Admin" }] },
    { "label": "AlarmCfg", "children": [{ "label": "AlarmCfg ReadOnly" },{ "label": "AlarmCfg Admin" }] },
    { "label": "BI", "children": [{ "label": "BI ReadOnly" },{ "label": "BI Admin" }] },
    { "label": "Launcher page", "children": [{ "label": " Launcher pageReadOnly" },{ "label": "Launcher page Admin" }] },
    { "label": "NetAn", "children": [{ "label": "NetAn ReadOnly" },{ "label": "NetAn Admin" }] },
    { "label": "RHEL", "children": [{ "label": "RHEL ReadOnly" },{ "label": "RHEL Admin" }] },
    { "label": "Self Service", "children": [{ "Self Service label": "ReadOnly" },{ "label": "Self Service Admin" }] },
    { "label": "TechPackIDE", "children": [{ "TechPackIDE label": "ReadOnly" },{ "label": "TechPackIDE Admin" }] },
    { "label": "User Management", "children": [{ "label": "User Management ReadOnly" },{ "label": "User Management Admin" }] },
    { "label": "Windows", "children": [{ "label": "Windows ReadOnly" },{ "label": "Windows Admin" }] },
  ];  

  //statup_functions
  setTimeout(()=>{ this.startup_fun();},550);
 ...
 didRender(){
   ...
   this.userdetails_container = this.shadowRoot.querySelector("#selected-user");
   ...
   this.statusChange_dialog= this.shadowRoot.querySelector("#status_dialog");
       ...
       setTimeout(()=>{ 
        ...
        this.filter_apps= this.shadowRoot.querySelector("#filter-form > e-um-apps-groups")
            .shadowRoot.querySelector("#apps").shadowRoot.querySelectorAll("div > ul > eui-tree-item");
        this.filter_groups= this.shadowRoot.querySelector("#filter-form > e-um-apps-groups")
             .shadowRoot.querySelector("#groups").shadowRoot.querySelectorAll("div > ul > eui-tree-item");
...
set_multiPanelTile(){
  ...
  let panel_subtitle = this.multi_panel_tile.shadowRoot.querySelector("div.tile__header__left__subtitle")
  let table_rows_count= `<span class="row_count">${this.data.length}</span><span> items</span>`;

  let filter_clear= `<span class="filter_text">Filters applied</span> - 
      <span id="filter_clear" style="color: #0069c2;cursor: pointer;">Clears</span>`;
  let last_refresh='<span refresh_text>Last refreshed</span> <span class="refresh_time">right now</span>';
  panel_subtitle.innerHTML =`${table_rows_count} | ${filter_clear} | ${last_refresh}`;
  let filter_clear_link = this.multi_panel_tile.shadowRoot.querySelector("#filter_clear");
  filter_clear_link.addEventListener("click",this);
 
}
//add style to table heading content
set_tableheadingstyle(){
  let last_tableheading =this.users_table.shadowRoot.querySelector("thead  th:nth-last-child(2)");
  let table_headings = this.users_table.shadowRoot.querySelectorAll("thead th span.table__cell-content");
  last_tableheading.style.borderRight="none";
  for(let heading of table_headings){
...
let count_element=this.multi_panel_tile.shadowRoot.querySelector("div.tile__header__left__subtitle > span.row_count");
count_element.innerHTML=row_count
}
//select table row count 
select_tablerowcount(select_row){
  let count_element=this.multi_panel_tile.shadowRoot.querySelector("div.tile__header__left__subtitle > span.filter_text");
  if(select_row > 0){
...
unselect_groupchildren(group_item){
  let children_items = group_item.querySelectorAll("eui-tree-item");
  console.log(children_items);
  for(let item of children_items){
 ...
 filtering_usertable(formdata){
   ...
   let new_tabledata=[];
    ...
    new_tabledata=this.data.filter(row=>{
      ...
      for(let column of this.table_columns){
       
      //console.log(row[column.attribute]+ "::"+ formdata[column.title.toLowerCase()]);
      let rowcolumn_data = row[column.attribute].toLowerCase();
...
filterform_data(){
  let formdata={};
  let text_fields =  this.filter_form.querySelectorAll("eui-text-field");
  let status_checkboxs = this.filter_form.querySelectorAll("#status > eui-checkbox");
  ...
  for(let app of this.filter_apps){
  ...
  formdata.group=[];
  for(let group of this.filter_groups){
    if(group.hasAttribute("checked") || group.hasAttribute("indeterminate")){
      let children_items = group.querySelectorAll("eui-tree-item[checked]");
      for(let item of children_items){
  ...
  console.log(formdata.groups);
   ...
   //filter form status
  formdata.status=[];
  for(let checkbox of status_checkboxs ){
...
reset_filterform(){
  ...
  //uncheck select apps
  for(let apps of this.filter_apps){
...
select_userdetails(user){
  ...
  if(user === undefined ){
    ...
    // console.log(user);
    let details = this.table_columns.map(column=>{
      if(column.title == "Status"){
        return `<div class="info_text">
        <div class="title">${column.title}</div>
        <div class="info-gray">
        <eui-icon name=${user[column.attribute] == "Disabled"? "cross":"check"}
           color=${user[column.attribute] == "Disabled"? "red":"green"}></eui-icon> 
...
user_actionbutton(val){
  let class_list = this.action_buttons.classList;
  if(val){
...
let updated_data = this.data.map(dataitem =>{
  console.log(dataitem);
  if(dataitem.col1 == user.col1){ dataitem.col7=status;}  
    return  dataitem;
});
...
changeUser_status(user){
  ...
  let dialogbox = this.statusChange_dialog;
  let dialog_centent =this.statusChange_dialog.querySelector("div.dialog-content");
  let dialog_confirmbutton =this.statusChange_dialog.querySelector("eui-button");
...
user_delete(user){
  ...
  let new_userlist = this.data.filter(dataitem=>{
...
// component Event handling
  ...
  if(event.type ==="eui-tree:select"&& event.target.id ==="filterapps"){
    ...
    console.log(event.detail);
     if(event.detail.selected){
      ...
      let select_apps = this.select_filterapps.filter(app=>{
        if(app !== event.detail.label){ return app;} 
      });
 ...
 if(event.type ==="eui-dialog:cancel" && event.target.id ==="status_dialog"){
    console.log("user dialog cancel");
    console.log(event);
    let confirm_button = event.target.querySelector("eui-button");
