...
export default class SsSetupPasswordless extends LitComponent {
...
setupPasswordless_dialog(){
  let dialog_content = this.dialogbox.querySelector("div.dialog-content");
  let dialogSetup_button = this.dialogbox.querySelector("eui-button");
...
handleEvent(event){
  if(event.type === 'eui-dialog:cancel' && event.target.id === 'dialog'){
    let dialog_button = event.target.querySelector("eui-button");
        ...
        <section class="passwordless-login">
            ...
            <div class="content-block">
                <div class="fields column-1">
                   <p>Passwordless login setup is available only for ENIQ-S RHEL server access.<br>
                   The user should be pre-existing in  RHEL before initiciting this passwordless setup</p>
                </div>
            </div>
        </section>
          ...
          <div class="content-block">
              <div class="fields column-1">
                    <div class="form-fields">
                        <eui-button id="setup-passwordless" primary @click="${this}"
                          class="${this.have_passwordlessLogin ? 'hide': ''}">
                          Setup passwordless login 
                        </eui-button>
                        <eui-button class="hide1" id="revoke-passwordless" primary @click="${this}"
                        class="${this.have_passwordlessLogin ? '': 'hide'}">
