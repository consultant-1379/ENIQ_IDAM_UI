...
define(["lodash", "jquery", "org/forgerock/commons/ui/common/main/Router", "org/forgerock/openam/ui/common/components/TreeNavigation", "org/forgerock/openam/ui/user/uma/services/UMAService"], function (_, $, Router, TreeNavigation, UMAService) {
    var LabelTreeNavigationView = TreeNavigation.extend({
        ...
        render: function render(args, callback) {
            ...
            UMAService.labels.all().done(function (data) {
                ...
                userLabels = _.filter(data.result, function (label) {
                    return label.type.toLowerCase() === "user";
                });
                sortedUserLabels = _.sortBy(userLabels, function (label) {
                    return label.name;
                });

                self.data.labels = {
                    starred: _.filter(data.result, function (label) {
                        return label.type.toLowerCase() === "starred";
                    }),
                    system: _.filter(data.result, function (label) {
                        return label.type.toLowerCase() === "system";
                    }),
        ...
        addToParent: function addToParent(collection, label) {
        ...
        addUserLabels: function addUserLabels(userLabels) {
            var self = this;

            this.data.nestedLabels = [];
            this.data.labels.user = _.sortBy(userLabels, function (label) {
                return label.name;
            });
...
//# sourceMappingURL=LabelTreeNavigationView.js.map
