...
define(["jquery", "lodash", "org/forgerock/commons/ui/common/main/AbstractView", "org/forgerock/commons/ui/common/util/UIUtils", "org/forgerock/openam/ui/admin/services/realm/PoliciesService", "org/forgerock/openam/ui/admin/views/realms/authorization/policies/conditions/ConditionAttrBooleanView", "org/forgerock/openam/ui/admin/views/realms/authorization/policies/conditions/ConditionAttrArrayView", "org/forgerock/openam/ui/admin/views/realms/authorization/policies/conditions/ConditionAttrStringView", "org/forgerock/openam/ui/admin/views/realms/authorization/policies/conditions/ConditionAttrObjectView", "org/forgerock/openam/ui/admin/views/realms/authorization/policies/conditions/ConditionAttrEnumView", "org/forgerock/openam/ui/admin/views/realms/authorization/policies/conditions/ConditionAttrTimeView", "org/forgerock/openam/ui/admin/views/realms/authorization/policies/conditions/ConditionAttrDayView", "org/forgerock/openam/ui/admin/views/realms/authorization/policies/conditions/ConditionAttrDateView", "handlebars"], function ($, _, AbstractView, UIUtils, PoliciesService, BooleanAttr, ArrayAttr, StringAttr, ObjectAttr, EnumAttr, TimeAttr, DayAttr, DateAttr, Handlebars) {
    return AbstractView.extend({
        ...
        render: function render(schema, element, itemID, itemData, callback) {
            var self = this,
                hiddenData = {};
        ...
        createListItem: function createListItem(allEnvironments, item) {
            var self = this,
                itemToDisplay = null,
                ...
                if (type === self.SCRIPT_RESOURCE) {
                    ...
                    _.each(mergedData, function (val, key) {
                        if (key === "type") {
                            ...
                            itemToDisplay["" + self.i18n.condition.key + type + self.i18n.condition.props + key] = $.t("" + self.i18n.condition.key + type + self.i18n.condition.props + val);
                        } else {
        ...
        setListItemHtml: function setListItemHtml(item, itemToDisplay) {
            var self = this;
            ...
            if (itemData) {
                ...
                setTimeout(function () {
                    ...
                    if (_.isEmpty(helperText)) {
                        ...
                        self.$el.find(".info-button").show().attr("data-title", helperText.title.toString()).attr("data-content", helperText.content.toString()).popover();
                    }
        ...
        getHelperText: function getHelperText(schema) {
            var helperText = {};
            switch (schema.title) {
        ...
        buildHTML: function buildHTML(itemData, hiddenData, schema) {
            var self = this,
        ...
        setDefaultJsonValues: function setDefaultJsonValues(schema) {
        ...
        animateOut: function animateOut() {
            ...
            this.$el.find(".field-float-pattern, .field-float-selectize, .timezone-field").find("label").removeClass("showLabel").next("input").addClass("placeholderText");

            this.$el.find(".field-float-select select:not(.type-selection)").addClass("placeholderText").prev("label").removeClass("showLabel");
        ...
        animateIn: function animateIn() {
            var self = this;
            setTimeout(function () {
                self.$el.find(".field-float-pattern, .field-float-selectize, .timezone-field").find("label").addClass("showLabel").next("input, div input").removeClass("placeholderText").prop("readonly", false);

                self.$el.find(".field-float-select select:not(.type-selection)").removeClass("placeholderText").prop("readonly", false).prev("label").addClass("showLabel");
            }, 10);
        }
    });
});
//# sourceMappingURL=EditEnvironmentView.js.map
