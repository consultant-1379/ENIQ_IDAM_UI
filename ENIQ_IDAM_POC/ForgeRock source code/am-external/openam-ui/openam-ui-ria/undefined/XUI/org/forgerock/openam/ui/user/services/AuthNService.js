 ...
 */
define(["jquery", "lodash", "org/forgerock/commons/ui/common/components/Messages", "org/forgerock/commons/ui/common/main/AbstractDelegate", "org/forgerock/commons/ui/common/main/Configuration", "org/forgerock/commons/ui/common/main/EventManager", "org/forgerock/commons/ui/common/util/URIUtils", "org/forgerock/openam/ui/common/services/fetchUrl", "org/forgerock/openam/ui/common/util/Constants", "org/forgerock/openam/ui/user/login/tokens/AuthenticationToken", "store/modules/local/session", "store/index", "org/forgerock/openam/ui/common/util/uri/query"], function ($, _, Messages, AbstractDelegate, Configuration, EventManager, URIUtils, fetchUrl, Constants, AuthenticationToken, session, store, query) {
     ...
     */
    var createAuthenticateURL = function createAuthenticateURL() {
        var parameters = handleFragmentParameters(query.parseParameters(URIUtils.getCurrentFragmentQueryString()));
        /**
        ...
        return addQueryStringToUrl(fetchUrl.default("/authenticate", { realm: store.default.getState().remote.info.realm }), query.urlParamsFromObject(parameters));
    };
    ...
    obj.begin = function (options) {
        ...
        return obj.serviceCall(serviceCall).then(function (requirements) {
            return requirements;
        }, function (jqXHR) {
    ...
    obj.handleRequirements = function (requirements) {
        ...
        var isAuthenticated = requirements.hasOwnProperty("tokenId");
    ...
    obj.submitRequirements = function (requirements, options) {
        var timeOutErrorCode = "110";
        var processSucceeded = function processSucceeded(requirements) {
            obj.handleRequirements(requirements);
            return requirements;
        };
        var processFailed = function processFailed(reason) {
            var failedStage = requirementList.length;
            obj.resetProcess();
            return [failedStage, reason];
        };
        var goToFailureUrl = function goToFailureUrl(errorBody) {
        ...
        _.assign(serviceCall, options);
        return obj.serviceCall(serviceCall).then(processSucceeded, function (jqXHR) {
            var oldReqs = void 0;
            var errorBody = void 0;
            var currentStage = requirementList.length;
            var message = void 0;
            var reasonThatWillNotBeDisplayed = 1;
            ...
            if (_.get(jqXHR, "responseJSON.detail.errorCode") === timeOutErrorCode) {
                ...
                Messages.addMessage({ message: message, type: Messages.TYPE_DANGER });
            } else {
                // we have a 401 unauthorized response
    ...
    function hasRealmChanged() {
        ...
        return auth.subRealm !== knownAuth.subRealm || _.get(auth, "urlParams.realm") !== _.get(knownAuth, "urlParams.realm");
    }
    function hasAuthIndexChanged() {
        var auth = Configuration.globalData.auth;
        return _.get(auth, "urlParams.authIndexType") !== _.get(knownAuth, "urlParams.authIndexType") || _.get(auth, "urlParams.authIndexValue") !== _.get(knownAuth, "urlParams.authIndexValue");
    }
    obj.getRequirements = function (args) {
        if (AuthenticationToken.get()) {
            var paramsWithAuthToken = _.extend({ authId: AuthenticationToken.get() }, Configuration.globalData.auth.urlParams);
            AuthenticationToken.remove();
            return obj.submitRequirements(paramsWithAuthToken).done(function () {
                knownAuth = _.clone(Configuration.globalData.auth);
            });
...
//# sourceMappingURL=AuthNService.js.map
