...
define(["jquery", "lodash", "backbone", "backbone.paginator", "backgrid-filter", "org/forgerock/commons/ui/common/backgrid/Backgrid", "org/forgerock/commons/ui/common/backgrid/extension/ThemeablePaginator", "org/forgerock/commons/ui/common/main/Configuration", "org/forgerock/commons/ui/common/main/EventManager", "org/forgerock/commons/ui/common/main/Router", "org/forgerock/commons/ui/common/util/Constants", "org/forgerock/openam/ui/admin/models/authorization/PolicySetModel", "org/forgerock/openam/ui/admin/views/realms/authorization/common/AbstractListView", "org/forgerock/openam/ui/admin/services/realm/PoliciesService", "org/forgerock/openam/ui/common/util/BackgridUtils", "org/forgerock/openam/ui/common/util/RealmHelper", "org/forgerock/openam/ui/common/util/URLHelper"], function ($, _, Backbone, BackbonePaginator, BackgridFilter, Backgrid, ThemeablePaginator, Configuration, EventManager, Router, Constants, PolicySetModel, AbstractListView, PoliciesService, BackgridUtils, RealmHelper, URLHelper) {
    return AbstractListView.extend({
        ...
        render: function render(args, callback) {
            ...
            PoliciesService.listResourceTypes().then(_.bind(function (resourceTypes) {
                ...
                if (resourceTypes.resultCount < 1) {
                    ...
                    var PolicySets = Backbone.PageableCollection.extend({
                        url: URLHelper.substitute("__api__/applications"),
                        model: PolicySetModel,
                        state: BackgridUtils.getState(),
                        queryParams: BackgridUtils.getQueryParams({
                            filterName: "eq",
                            _queryFilter: ["name+eq+" + encodeURIComponent('"^(?!sunAMDelegationService$).*"')]
                        }),
                    ...
                    this.data.items.fetch({ reset: true }).done(function (response) {
                        if (response.resultCount > 0) {
                            _this.data.hasPolicySets = true;
                            _this.renderTable(callback);
                        } else {
                            _this.data.hasPolicySets = false;
                            _this.parentRender(_this.renderToolbar);
                        }
                    }).fail(function () {
        ...
        renderTable: function renderTable(callback) {
            var _this2 = this;

            var self = this;
            var ClickableRow = BackgridUtils.ClickableRow.extend({
            ...
            var columns = [{
                ...
                cell: BackgridUtils.TemplateCell.extend({
                    ...
                    deleteItem: function deleteItem(e) {
                        self.onDeleteClick(e, { type: $.t("console.authorization.common.policySet") }, this.model.id);
                    }
                }),
                sortable: false,
                editable: false
            ...
            var paginator = new Backgrid.Extension.ThemeablePaginator({
                collection: this.data.items,
                windowSize: 3
            });
            ...
            this.parentRender(function () {
                ...
                _this2.$el.find(".table-container").append(grid.render().el);
                _this2.$el.find(".panel-body").append(paginator.render().el);
        ...
        startImportPolicies: function startImportPolicies() {
            this.$el.find("[name=upload]").trigger("click");
        },
        importPolicies: function importPolicies(e) {
            PoliciesService.importPolicies(e.target.result).done(function () {
        ...
        readImportFile: function readImportFile() {
        ...
        exportPolicies: function exportPolicies() {
            var realm = this.realmPath === "/" ? "" : RealmHelper.encodeRealm(this.realmPath);
            this.$el.find("[data-export-policies]").attr("href", "" + Constants.host + Constants.context + "/xacml" + realm + "/policies");
        },
        ...
        addNewPolicySet: function addNewPolicySet() {
...
//# sourceMappingURL=PolicySetsView.js.map
