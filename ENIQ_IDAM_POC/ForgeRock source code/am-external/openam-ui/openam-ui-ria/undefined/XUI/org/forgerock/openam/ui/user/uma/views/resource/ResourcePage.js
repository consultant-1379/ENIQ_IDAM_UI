...
define(["jquery", "lodash", "org/forgerock/commons/ui/common/main/AbstractView", "backbone", "org/forgerock/commons/ui/common/backgrid/Backgrid", "org/forgerock/openam/ui/common/util/BackgridUtils", "org/forgerock/commons/ui/common/components/BootstrapDialog", "org/forgerock/openam/ui/user/uma/views/share/CommonShare", "org/forgerock/commons/ui/common/util/Constants", "org/forgerock/commons/ui/common/main/EventManager", "org/forgerock/openam/ui/user/uma/views/resource/LabelTreeNavigationView", "org/forgerock/commons/ui/common/components/Messages", "org/forgerock/commons/ui/common/main/Router", "org/forgerock/commons/ui/common/util/UIUtils", "org/forgerock/openam/ui/user/uma/services/UMAService", "org/forgerock/openam/ui/user/uma/models/UMAResourceSetWithPolicy",

// jquery dependencies
"selectize"], function ($, _, AbstractView, Backbone, Backgrid, BackgridUtils, BootstrapDialog, CommonShare, Constants, EventManager, LabelTreeNavigationView, Messages, Router, UIUtils, UMAService, UMAResourceSetWithPolicy) {
    ...
    function createLabels(labelNames) {
        ...
        return $.when.apply($, _toConsumableArray(creationPromises)).then(function () {
    ...
    var ResourcePage = AbstractView.extend({
        ...
        onModelError: function onModelError(model, response) {
            console.error("Unrecoverable load failure UMAResourceSetWithPolicy. " + response.responseJSON.code + " (" + response.responseJSON.reason + ")" + response.responseJSON.message);
        },
        ...
        onShare: function onShare() {
            ...
            shareView.renderDialog({
                ...
                share: function share() {
                    ...
                    _this.updateUnshareButton();
                }
            });
        },
        onToggleStarred: function onToggleStarred() {
            var self = this,
        ...
        renderLabelsOptions: function renderLabelsOptions() {
            var self = this,
                ...
                render: {
                    item: function item(_item) {
                        return "<div data-value=\"" + _item.name + "\" class=\"item\">" + _item.name + "</div>\"";
                    }
                },
                labelField: "name",
                valueField: "name",
        ...
        updateLabelOptions: function updateLabelOptions() {
            var labelsSelectize = this.getLabelSelectize(),
                userLabels = _.filter(this.allLabels, isUserLabel),
                resourceUserLabelNames = _(this.model.get("labels")).map(_.partial(getLabelForId, this.allLabels)).filter(isUserLabel).sortBy("name").pluck("name").value();
        ...
        renderSelectizeCell: function renderSelectizeCell() {
        ...
        render: function render(args, callback) {
            ...
            if (this.model && this.id === id) {
                ...
                $.when(getAllLabels(), UMAResourceSetWithPolicy.findOrCreate({ _id: id }).fetch()).done(function (allLabels, model) {
        ...
        renderWithModel: function renderWithModel(callback) {
            var collection,
                grid,
                RevokeCell,
                self = this;
        ...
        getLabelSelectize: function getLabelSelectize() {
            return this.$el.find("#labels select")[0].selectize;
        },
        stopEditingLabels: function stopEditingLabels() {
        ...
        editLabels: function editLabels() {
            var labelsSelect = this.getLabelSelectize();
            labelsSelect.enable();
            labelsSelect.focus();
        ...
        disableLabelControls: function disableLabelControls() {
            var labelsSelect = this.getLabelSelectize();
            labelsSelect.disable();
            this.$el.find("button#saveLabels").prop("disabled", true);
            this.$el.find("button#discardLabels").prop("disabled", true);
        },
        enableLabelControls: function enableLabelControls() {
            var labelsSelect = this.getLabelSelectize();
            labelsSelect.enable();
            this.$el.find("button#saveLabels").prop("disabled", false);
            this.$el.find("button#discardLabels").prop("disabled", false);
        },
        submitLabelsChanges: function submitLabelsChanges() {
            var self = this,
                ...
                existingUserLabelIds = _(existingUserLabelNames).map(_.partial(getLabelForName, userLabels)).pluck("_id").value(),
                existingNonUserLabelIds = _(self.model.get("labels")).map(_.partial(getLabelForId, this.allLabels)).reject(isUserLabel).pluck("_id").value(),
        ...
        discardLabelsChanges: function discardLabelsChanges() {
            this.stopEditingLabels();
            this.updateLabelOptions();
        },
        updateUnshareButton: function updateUnshareButton() {
