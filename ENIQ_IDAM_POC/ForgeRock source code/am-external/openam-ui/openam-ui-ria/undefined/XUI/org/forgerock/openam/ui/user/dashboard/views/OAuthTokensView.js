...
define(["jquery", "lodash", "org/forgerock/commons/ui/common/main/AbstractView", "org/forgerock/openam/ui/user/dashboard/services/OAuthTokensService"], function ($, _, AbstractView, OAuthTokensService) {
    var OAuthToken = AbstractView.extend({
            ...
            OAuthTokensService.getApplications().then(function (data) {
                self.data.applications = _.map(data.result, function (application) {
                    return {
                        ...
                        expiryDateTime: application.expiryDateTime ? new Date(application.expiryDateTime).toLocaleString() : $.t("openam.oAuth2.tokens.neverExpires")
                    };
                });
        ...
        deleteToken: function deleteToken(event) {
