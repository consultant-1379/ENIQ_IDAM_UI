...
define(["jquery", "lodash", "org/forgerock/commons/ui/common/main/AbstractView", "org/forgerock/commons/ui/common/util/Constants", "org/forgerock/commons/ui/common/main/EventManager", "org/forgerock/openam/ui/admin/views/realms/authentication/chains/EditLinkView", "org/forgerock/openam/ui/admin/utils/FormHelper", "handlebars", "org/forgerock/openam/ui/admin/views/realms/authentication/chains/LinkView", "org/forgerock/commons/ui/common/components/Messages", "org/forgerock/openam/ui/admin/views/realms/authentication/chains/PostProcessView", "org/forgerock/commons/ui/common/main/Router", "org/forgerock/openam/ui/admin/services/realm/AuthenticationService",
// jquery dependencies
"sortable"], function ($, _, AbstractView, Constants, EventManager, EditLinkView, FormHelper, Handlebars, LinkView, Messages, PostProcessView, Router, AuthenticationService) {
    var createLinkView = function createLinkView(index, view) {
        ...
        initSortable = function initSortable(self) {
        self.$el.find("ol#sortableAuthChain").nestingSortable({
            ...
            onDrag: function onDrag(item, position) {
                item.css({
                    left: position.left - self.adjustment.left,
                    top: position.top - self.adjustment.top
                });
            },
            onDragStart: function onDragStart(item, container) {
    ...
    return AbstractView.extend({
        ...
        addItemToList: function addItemToList(element) {
        ...
        editItem: function editItem(linkview) {
        ...
        onDeleteClick: function onDeleteClick(e) {
            ...
            FormHelper.showConfirmationBeforeDeleting({ type: $.t("console.authentication.modules.chain") }, _.bind(this.deleteChain, this));
        },
        deleteChain: function deleteChain() {
            var self = this;

            AuthenticationService.authentication.chains.remove(self.data.realmPath, self.data.form.chainData._id).then(function () {
                Messages.addMessage({
        ...
        render: function render(args) {
            ...
            AuthenticationService.authentication.chains.get(args[0], args[1]).then(function (data) {
                self.data = {
                    realmPath: args[0],
        ...
        saveSettings: function saveSettings() {
            ...
            PostProcessView.addClassNameDialog().then(function () {
                ...
                AuthenticationService.authentication.chains.update(self.data.realmPath, chainData._id, savedData).then(function () {
        ...
        saveChain: function saveChain() {
            var chainData = this.data.form.chainData,
                savedData = {
                authChainConfiguration: chainData.authChainConfiguration
            };

            AuthenticationService.authentication.chains.update(this.data.realmPath, chainData._id, savedData).then(function () {
                EventManager.sendEvent(Constants.EVENT_DISPLAY_MESSAGE_REQUEST, "changesSaved");
            }, function (response) {
                Messages.addMessage({
                    type: Messages.TYPE_DANGER,
        ...
        sortChainData: function sortChainData(from, to) {
        ...
        validateChain: function validateChain() {
            var invalid = false,
                alert = "",
                firstRequiredIndex,
                sufficentIndex,
            ...
            if (config.length === 0) {
                ...
                if (firstRequiredIndex > -1 && sufficentIndex > -1 && firstRequiredIndex < sufficentIndex && config.length - 1 > sufficentIndex) {
                    alert = Handlebars.compile("{{> alerts/_Alert type='warning' " + "text='console.authentication.editChains.alerts.reqdFailSuffPass'}}");
                }
...
//# sourceMappingURL=EditChainView.js.map
