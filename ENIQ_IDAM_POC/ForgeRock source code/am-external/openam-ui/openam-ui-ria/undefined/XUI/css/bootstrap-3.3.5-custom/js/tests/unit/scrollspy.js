...
$(function () {
  ...
  QUnit.test('should only switch "active" class on current target', function (assert) {
    ...
    var sectionHTML = '<div id="root" class="active">' + '<div class="topbar">' + '<div class="topbar-inner">' + '<div class="container" id="ss-target">' + '<ul class="nav">' + '<li><a href="#masthead">Overview</a></li>' + '<li><a href="#detail">Detail</a></li>' + '</ul>' + '</div>' + '</div>' + '</div>' + '<div id="scrollspy-example" style="height: 100px; overflow: auto;">' + '<div style="height: 200px;">' + '<h4 id="masthead">Overview</h4>' + '<p style="height: 200px">' + 'Ad leggings keytar, brunch id art party dolor labore.' + '</p>' + '</div>' + '<div style="height: 200px;">' + '<h4 id="detail">Detail</h4>' + '<p style="height: 200px">' + 'Veniam marfa mustache skateboard, adipisicing fugiat velit pitchfork beard.' + '</p>' + '</div>' + '</div>' + '</div>';
    var $section = $(sectionHTML).appendTo('#qunit-fixture');

    var $scrollspy = $section.show().find('#scrollspy-example').bootstrapScrollspy({ target: '#ss-target' });
  ...
  QUnit.test('should correctly select middle navigation option when large offset is used', function (assert) {
    ...
    var sectionHTML = '<div id="header" style="height: 500px;"></div>' + '<nav id="navigation" class="navbar">' + '<ul class="nav navbar-nav">' + '<li class="active"><a id="one-link" href="#one">One</a></li>' + '<li><a id="two-link" href="#two">Two</a></li>' + '<li><a id="three-link" href="#three">Three</a></li>' + '</ul>' + '</nav>' + '<div id="content" style="height: 200px; overflow-y: auto;">' + '<div id="one" style="height: 500px;"></div>' + '<div id="two" style="height: 300px;"></div>' + '<div id="three" style="height: 10px;"></div>' + '</div>';
    var $section = $(sectionHTML).appendTo('#qunit-fixture');
  ...
  QUnit.test('should add the active class to the correct element', function (assert) {
    assert.expect(2);
    var navbarHtml = '<nav class="navbar">' + '<ul class="nav">' + '<li id="li-1"><a href="#div-1">div 1</a></li>' + '<li id="li-2"><a href="#div-2">div 2</a></li>' + '</ul>' + '</nav>';
    ...
    var $content = $(contentHtml).appendTo('#qunit-fixture').bootstrapScrollspy({ offset: 0, target: '.navbar' });

    var testElementIsActiveAfterScroll = function testElementIsActiveAfterScroll(element, target) {
    ...
    $.when(testElementIsActiveAfterScroll('#li-1', '#div-1')).then(function () {
      return testElementIsActiveAfterScroll('#li-2', '#div-2');
    });
  });
  ...
  QUnit.test('should add the active class correctly when there are nested elements at 0 scroll offset', function (assert) {
    ...
    var times = 0;
    var done = assert.async();
    var navbarHtml = '<nav id="navigation" class="navbar">' + '<ul class="nav">' + '<li id="li-1"><a href="#div-1">div 1</a>' + '<ul>' + '<li id="li-2"><a href="#div-2">div 2</a></li>' + '</ul>' + '</li>' + '</ul>' + '</nav>';
    ...
    var $content = $(contentHtml).appendTo('#qunit-fixture').bootstrapScrollspy({ offset: 0, target: '#navigation' });
  ...
  QUnit.test('should clear selection if above the first section', function (assert) {
    ...
    var sectionHTML = '<div id="header" style="height: 500px;"></div>' + '<nav id="navigation" class="navbar">' + '<ul class="nav navbar-nav">' + '<li class="active"><a id="one-link" href="#one">One</a></li>' + '<li><a id="two-link" href="#two">Two</a></li>' + '<li><a id="three-link" href="#three">Three</a></li>' + '</ul>' + '</nav>';
    $(sectionHTML).appendTo('#qunit-fixture');

    var scrollspyHTML = '<div id="content" style="height: 200px; overflow-y: auto;">' + '<div id="spacer" style="height: 100px;"/>' + '<div id="one" style="height: 100px;"/>' + '<div id="two" style="height: 100px;"/>' + '<div id="three" style="height: 100px;"/>' + '<div id="spacer" style="height: 100px;"/>' + '</div>';
    ...
    $scrollspy.bootstrapScrollspy({
      ...
      $scrollspy.one('scroll.bs.scrollspy', function () {
  ...
  QUnit.test('should correctly select navigation element on backward scrolling when each target section height is 100%', function (assert) {
    assert.expect(5);
    var navbarHtml = '<nav class="navbar">' + '<ul class="nav">' + '<li id="li-100-1"><a href="#div-100-1">div 1</a></li>' + '<li id="li-100-2"><a href="#div-100-2">div 2</a></li>' + '<li id="li-100-3"><a href="#div-100-3">div 3</a></li>' + '<li id="li-100-4"><a href="#div-100-4">div 4</a></li>' + '<li id="li-100-5"><a href="#div-100-5">div 5</a></li>' + '</ul>' + '</nav>';
