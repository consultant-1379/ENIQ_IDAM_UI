...
$(function () {
  ...
  QUnit.test('should activate element by tab id', function (assert) {
    assert.expect(2);
    var tabsHTML = '<ul class="tabs">' + '<li><a href="#home">Home</a></li>' + '<li><a href="#profile">Profile</a></li>' + '</ul>';
  ...
  QUnit.test('should activate element by tab id', function (assert) {
    assert.expect(2);
    var pillsHTML = '<ul class="pills">' + '<li><a href="#home">Home</a></li>' + '<li><a href="#profile">Profile</a></li>' + '</ul>';
  ...
  QUnit.test('should not fire shown when show is prevented', function (assert) {
    ...
    $('<div class="tab"/>').on('show.bs.tab', function (e) {
      e.preventDefault();
  ...
  QUnit.test('show and shown events should reference correct relatedTarget', function (assert) {
    ...
    var dropHTML = '<ul class="drop">' + '<li class="dropdown"><a data-toggle="dropdown" href="#">1</a>' + '<ul class="dropdown-menu">' + '<li><a href="#1-1" data-toggle="tab">1-1</a></li>' + '<li><a href="#1-2" data-toggle="tab">1-2</a></li>' + '</ul>' + '</li>' + '</ul>';

    $(dropHTML).find('ul > li:first a').bootstrapTab('show').end().find('ul > li:last a').on('show.bs.tab', function (e) {
  ...
  QUnit.test('should fire hide and hidden events', function (assert) {
    ...
    $(tabsHTML).find('li:first a').on('hide.bs.tab', function () {
      assert.ok(true, 'hide event fired');
    }).bootstrapTab('show').end().find('li:last a').bootstrapTab('show');

    $(tabsHTML).find('li:first a').on('hidden.bs.tab', function () {
  ...
  QUnit.test('should not fire hidden when hide is prevented', function (assert) {
    ...
    $(tabsHTML).find('li:first a').on('hide.bs.tab', function (e) {
      e.preventDefault();
      assert.ok(true, 'hide event fired');
      done();
  ...
  QUnit.test('selected tab should have aria-expanded', function (assert) {
    assert.expect(8);
    var tabsHTML = '<ul class="nav nav-tabs">' + '<li class="active"><a href="#home" toggle="tab" aria-expanded="true">Home</a></li>' + '<li><a href="#profile" toggle="tab" aria-expanded="false">Profile</a></li>' + '</ul>';
