...
import { getProductIndex, expandProductsPage } from '../../utils/utils.js';
...
describe('Keyboard Navigation', () => {
  ...
  it('can open the product page from an active card (in focus) by pressing the Enter key', async () => {
    ...
    await pressTabNTimes(clickedItemIndex + 1);
  ...
  it('app description can be toggled by pressing the Space key', async () => {
    ...
    const neededTabPressCount = 2 + appCardIndex; // number of Tabs: from favoriteSelector to layoutSelector + to reach Card with given index
  ...
  it('can navigate between apps of different app groups by pressing Tab key', async () => {
    ...
    await pressTabNTimes(lengthOfFirstAppGroup);
