...
import ecmConfig from '../../test-utils/mockdata/cloud.manager.config.json';
import testAppConfig from '../../test-utils/mockdata/test-app.config.json';
import appViewLocale from '../../../src/components/app-view/locale/en-us.json';
...
describe('AppView Component Tests', () => {
  ...
  const beforeFunction = async () => {
    storageModuleGetStub.withArgs('groupingType').returns(null);
    storageModuleGetStub.withArgs('viewType').returns(null);
  ...
  before(async () => {
    storageModuleGetStub = sinon.stub(storage, 'get');
    route = stubRouter();
    AppView.register();
  });
  ...
  describe('Basic application setup', () => {
    ...
    it(`should render ${eeaGroups.length} card containers`, () => {
      const categories = appView.shadowRoot.querySelectorAll(CONSTANTS.eCardContainer);

      expect(Array.from(categories)).to.have.lengthOf(eeaGroups.length);
    });
  ...
  describe('A-Z grouping', () => {
    before(async () => {
      storageModuleGetStub.withArgs('groupingType').returns(GROUPPING_TYPE.ALPHABETICAL);
      const productName = eeaGroups[0].name;
      appView = await renderAppView({
        productName,
