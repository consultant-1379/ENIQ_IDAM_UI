...
import ConfigUtils from '../utils/ConfigUtil.js';
...
class UiConfigQueryService extends ConfigQueryService {
  constructor(options) {
    ...
    configManager.on(
      'config-changed',
      ({ name }) =>
        name === (CONSTANTS.MANUAL_CONFIG_NAME || CONSTANTS.MANUAL_OVERRIDES) &&
        this.updateConfig(),
    );
  }
  ...
  overrideConfig(resultConfig, overrides) {
    const mismatchedOverrides = { apps: [], groups: [], components: [], groupMappings: [] };
    if (overrides) {
  ...
  updateConfig() {
    ...
    const { resultConfig, mismatchedOverrides } = this.overrideConfig(
      ConfigUtils.mergeConfigs(configList),
      overrides,
    );
