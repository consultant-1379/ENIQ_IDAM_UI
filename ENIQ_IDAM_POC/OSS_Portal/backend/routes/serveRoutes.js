...
export default () => {
  router.get(apiConfig.serve.routes.packages, (_req, res) =>
    res.json(configQueryService.getPackages()),
  );

  router.get(apiConfig.serve.routes.importMap, (_req, res) =>
    res.json(configQueryService.getImportMap()),
  );

  router.use(apiConfig.serve.routes.static, proxyService.getMiddleware());
