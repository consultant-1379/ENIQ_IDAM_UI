...
const URLS = ['apps', 'components', 'groups'];
...
describe('Component tests for app.js', () => {
  ...
  const loadServerChokidar = { libName: PARAMS.chokidar, isDefault: true, impl: chokidarMock };
  ...
  describe('Testing state with two valid clients with different labels', () => {
    describe('Testing client with first label', () => {
      ...
      before(async function () {
        ...
        const localConfigMock = {
          libName: 'fs',
          isDefault: false,
          impl: fsMock({ uiServiceConfig: { k8sLabelValue: 'workspace-gui-1' } }),
        };
    ...
    describe('Testing client with second label', () => {
      before(async () => {
        const localConfigMock = {
          libName: 'fs',
          impl: fsMock({ uiServiceConfig: { k8sLabelValue: 'workspace-gui-2' } }),
        };
  ...
  describe('Refresh endpoint tests', () => {
    ...
    describe('Refresh propgation tests', () => {
      afterEach(() => {
        td.reset();
      });
