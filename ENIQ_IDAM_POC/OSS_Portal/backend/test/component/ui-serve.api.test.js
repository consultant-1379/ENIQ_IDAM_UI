...
describe('Component tests for ui-serve API', () => {
  ...
  beforeEach(async () => {
    ...
    request = await loadServer(
      ...
      { libName: 'chokidar', isDefault: true, impl: chokidarMock },
      { libName: '../../services/logging.js', isDefault: false, impl: loggingMock },
      localConfigMock,
    );
  });
  ...
  it('sends HTTP 200 for list-packages endpoint', async () => {
    await checkEndpoint(request, apiConfig.serve.routes.packages, 200);
  });

  it('sends HTTP 200 for import-map endpoint', async () => {
    await checkEndpoint(request, apiConfig.serve.routes.importMap, 200);
  });
  ...
  describe('Static Serve endpoint', () => {
    ...
    it('returns 400 "Bad Request" for invalid url', async () => {
      ...
      await checkEndpoint(request, `${apiConfig.serve.routes.static}/asdf`, 400);
    });

    it('returns 400 if filepath is missing', async () => {
      await requestDomainService(serviceObjectDomainApp1);
      await checkEndpoint(request, `${apiConfig.serve.routes.static}/domain1-1.0.0/`, 400);
    });

    it('returns 404 if service is not known', async () => {
      await requestDomainService(serviceObjectDomainApp1);
      await checkEndpoint(request, `${apiConfig.serve.routes.static}/domain2/file.png`, 404);
    });
    ...
    it('returns 200 if service and file is found', async () => {
      ...
      await checkEndpoint(
        request,
        `${apiConfig.serve.routes.static}/domain1-1.0.0/config.json`,
        200,
      );
    });
  });
