...
export default class LauncherPage extends LitComponent {
  ...
  didConnect() {
    ...
    this.products = ['ENIQ'];
    this.recent_apps = [
      { app: 'Alarm Monitor', favorite: true },
      { app: 'NetAn', favorite: false },
      { app: 'Network Health Monitor', favorite: true },
      { app: 'Payment', favorite: false },
    ];
    this.favorite_apps = [
      { app: 'Alarm Monitor', favorite: false },
      { app: 'Alarm Supervision Status', favorite: true },
      { app: 'Network Health Monitor', favorite: true },
      { app: 'PM Collection and Initiation', favorite: false },
    ];
  ...
  products_list = products => {
    const list_path = this.component_path.shadowRoot.querySelector(
      '.products > .products_list > .list',
    );
    let product_items;
    if (products.length > 0) {
      product_items = products.map(
        product => `
        <div class="list-item">
      ...
      apps_items = apps.map(
        app => `
          <div class="list-item" >
             ...
             <div class="icons">
                  <eui-tooltip action="" delay="0" message="${
                    app.app
                  }" position="bottom">
                   <eui-icon name="info"></eui-icon>
                  </eui-tooltip>
                  <eui-icon name="${
                    app.favorite ? 'favorite-solid' : 'favorite'
                  }"></eui-icon>
              </div>
          </div>`,
      );
  ...
  Recent_favorite_apps = (recent_apps, favorite_apps) => {
    // Recent apps
    const recent_apps_path = this.component_path.shadowRoot.querySelector(
      '.apps-block >.recent-app >.apps-list',
    );
    this.apps_list(recent_apps, recent_apps_path);
    // Favorite apps
    const favorite_apps_path = this.component_path.shadowRoot.querySelector(
      '.apps-block >.favorite-app >.apps-list',
    );
    ...
    return html`<div class=" launcher-page">
      ...
      <div class="search container">
        <div class="search_bar">
          <eui-text-field
            placeholder="Search all products and applications"
            name="item"
          >
